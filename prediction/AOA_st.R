...
train_sub =  train[sample(1:nrow(train), 5000), ]  
...
setwd("C:/Users/maike/Desktop/Carbon4D/GitHub_soil_temperature_moisture_4d_data/model_results/DI_AOA_5000")

#DI = trainDI(model=rfmodel)

#AOA <- aoa(predictors_spatially, rfmodel, trainDI = DI)

#save(DI,file = "DI.Rdata")

#save(AOA,file = "AOA_june.Rdata")

load("C:/Users/maike/Desktop/Carbon4D/GitHub_soil_temperature_moisture_4d_data/model_results/DI_AOA_5000/DI.Rdata")
...
plot(prediction, col=viridisLite::viridis(100),main="prediction with AOA",axes=F,legend=F)
plot(AOA$AOA,col=c("pink","transparent"),add=T,plg=list(x="topleft",box.col="black",bty="o",title="AOA"),axes=F,legend=F)
...
aoa_sub = aoa_crop==1
plot(aoa_sub)
values(aoa_sub)[values(aoa_sub) == 0] <- NA
plot(aoa_sub,col="green")

cols=viridisLite::viridis(80)
b <- seq(from=0,to=20,by=0.25)
