...
meteo_data$datetime <- as.POSIXct(strptime(meteo_data$datetime,format="%Y-%m-%d %H:%M:%S",tz="Etc/GMT-1"))
meteo_data$datetime[1]
...
plot(meteo_data$datetime,meteo_data$air_temperature_mountain)
lines(meteo_data$datetime,meteo_data$air_temperature_month,col="red",lwd=4)



#test = data.frame(meteo_data$datetime,meteo_data$air_temperature_mountain)
#names(test) = c("t","y")

#test = test[complete.cases(test$y), ]
...
#slope = do.call(rbind,lapply(seq(dim(test)[1]-windowsize),
#                     function(x) slopes(test[x:(x+windowsize),])))

#plot(test$y)
#lines(slope[,2]*1000000,col="red")
#plot(slope[,2],col="red")
...
predictor_set = merge(predictor_set,trends, by.x = c("date_hour"),by.y = c("datetime"), all.x = T)
...
radolan_sums$datetime = as.character(radolan_sums$datetime)

radolan_sums$datetime<- as.POSIXct(strptime(radolan_sums$datetime,format="%Y-%m-%d %H:%M:%S",tz="Etc/GMT-1"))



predictor_set = merge(predictor_set,radolan_sums, by.x = c("date_hour","probe_name"),by.y = c("datetime","probe_name"), all.x = T)
...
ext_soil_texture = extract(static_raster$soil_texture,probes$coordinates,df=T)
probes$soil_texture = ext_soil_texture$soil_texture

ext_soil_type = extract(static_raster$soil_type,probes$coordinates,df=T)
probes$soil_type = ext_soil_type$soil_type

ext_elevation = extract(static_raster$elevation,probes$coordinates,df=T)
probes$elevation = ext_elevation$elevation

ext_land_use = extract(static_raster$land_use,probes$coordinates,df=T)
probes$land_use = ext_land_use$land_use

ext_inclination = extract(static_raster$inclination,probes$coordinates,df=T)
probes$inclination = ext_inclination$inclination

ext_northness = extract(static_raster$northness,probes$coordinates,df=T)
probes$northness = ext_northness$northness

ext_eastness = extract(static_raster$eastness,probes$coordinates,df=T)
probes$eastness = ext_eastness$eastness

ext_topo_wetness = extract(static_raster$topo_wetness,probes$coordinates,df=T)
probes$topo_wetness = ext_topo_wetness$topo_wetness
...
predictor_set = predictor_set[ , -which(names(predictor_set) %in% c("air_temperature_valley_3",
...
for (i in 1:length(datetime_ndwi)) {
  datetime_ndwi_posix <- append(datetime_ndwi_posix, as.POSIXct(datetime_ndwi[i], format = "%Y%m%d%H%M%S",tz="Etc/GMT-1"))
}
datetime_ndwi_posix
...
for (i in 1:length(datetime_radar)) {
  datetime_radar_posix <- append(datetime_radar_posix, as.POSIXct(datetime_radar[i], format = "%Y%m%d%H%M%S",tz="Etc/GMT-1"))
}
...
ts_seq$datetime = as.POSIXct(strptime(ts_seq$datetime,"%Y-%m-%d %H:%M:%S",tz="Etc/GMT-1"))

probes_ndvi_t_2$time_ndvi = as.POSIXct(strptime(probes_ndvi_t_2$time_ndvi,"%Y-%m-%d %H:%M:%S",tz="Etc/GMT-1"))
...
probes_ndwi_t_2$time_ndwi = as.POSIXct(strptime(probes_ndwi_t_2$time_ndwi,"%Y-%m-%d %H:%M:%S",tz="Etc/GMT-1"))
...
probes_radar_t_2$time_radar = as.POSIXct(strptime(probes_radar_t_2$time_radar,"%Y-%m-%d %H:%M:%S",tz="Etc/GMT-1"))
...
predictor_set_2$date

head(predictor_set_2)

predictor_set_3 = predictor_set_2
# 
# data.table::uniqueN(predictor_set_3$probe_name)
# data.table::uniqueN(predictor_set_3$date)
# data.table::uniqueN(predictor_set_3$plot_id)
# 
# summary(predictor_set_3)

predictor_set_2$probe_name

predictor_set_2 = predictor_set_2[!duplicated(predictor_set_2[,c('probe_name', 'date_hour')]),]

summary(predictor_set_2)

##############################

predictor_set_2$trend_week <- round(predictor_set_2$trend_week,4)
predictor_set_2$trend_month <- round(predictor_set_2$trend_month,4)
predictor_set_2$trend_3month <- round(predictor_set_2$trend_3month,4)

predictor_set_2$air_temperature_day  <- round(predictor_set_2$air_temperature_day ,2)
predictor_set_2$air_temperature_week  <- round(predictor_set_2$air_temperature_week ,2)
predictor_set_2$air_temperature_month   <- round(predictor_set_2$air_temperature_month ,2)
predictor_set_2$air_temperature_3_month   <- round(predictor_set_2$air_temperature_3_month ,2)

predictor_set_2$inclination   <- round(predictor_set_2$inclination  ,2)
predictor_set_2$northness  <- round(predictor_set_2$northness ,2)
predictor_set_2$eastness    <- round(predictor_set_2$eastness  ,2)
predictor_set_2$topo_wetness    <- round(predictor_set_2$topo_wetness  ,2)

#############################

save(predictor_set_2,file = "C:/Users/maike/Desktop/Carbon4D/GitHub_soil_temperature_moisture_4d_data/predictor_set_complete_seasons_radolan_6.Rdata")
