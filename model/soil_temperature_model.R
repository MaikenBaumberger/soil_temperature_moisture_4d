...
trainIDs <- caret::createDataPartition(traindat$probe_name,p=0.05,list = FALSE) # Alle train-Daten so ca 390.000
# Auf ca 2180 Trainingsdaten kommen (==Anzahl Trainingsdaten im TopSoil-Model)!
traindat_sub <- traindat[trainIDs,]

timefolds <- CreateSpacetimeFolds(traindat_sub,spacevar = "probe_name")

# Dasselbe fuer Var-Daten:
varIDs <- createDataPartition(vardat$probe_name,p=0.1,list = FALSE) # Alle var-Daten so ca 120.000
...
save(traindat,traindat_sub,trainIDs, vardat,var_sub,varIDs, file = "Data_Model_DeepSoil.Rdata")
save(model_ffs_DS,var_sub,file = "Model_DeepSoil.Rdata")
